...
func main() {
	...
	// Defaults for local LGTM (make lgtm-up) so Zed ACP stdio works without
...
// applyDefaultOTELEnv fills unset OTEL_* vars for the local LGTM stack so
// hosts (e.g. Zed) need not configure the agent env by hand.
func applyDefaultOTELEnv() {
	if disabledOTEL() {
		// Ensure Init sees no endpoint even if .env set one earlier? Prefer
