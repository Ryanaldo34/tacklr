...
func (d *IR) Sheets() []Sheet {
	if g, ok := d.body.(*gridBody); ok {
		return g.sheets
	}
	return nil
}

func (d *IR) NamedRanges() []NamedRange {
	if g, ok := d.body.(*gridBody); ok {
		return g.named
	}
	return nil
...
func (d *IR) Cell(key, a1 string) (Cell, error) {
	g, ok := d.body.(*gridBody)
	if !ok {
		return Cell{}, ErrNotSupported
	}
	return g.Cell(key, a1)
}

func (d *IR) ReadRows(key string, start, end int) (Sheet, []string, error) {
	g, ok := d.body.(*gridBody)
	if !ok {
		return Sheet{}, nil, ErrNotSupported
	}
	return g.ReadRows(key, start, end)
}

func (d *IR) ReadRangeTSV(key, a1 string) (string, error) {
	g, ok := d.body.(*gridBody)
	if !ok {
		return "", ErrNotSupported
	}
	return g.ReadRangeTSV(key, a1)
}

func (d *IR) ReadCell(key, a1 string) (string, error) {
	g, ok := d.body.(*gridBody)
	if !ok {
		return "", ErrNotSupported
	}
	return g.ReadCell(key, a1)
}

func (d *IR) Tabs() []DocTab {
	if r, ok := d.body.(*richBody); ok {
		return r.tabs
	}
	return nil
}

func (d *IR) ReplaceBlock(id, text string, includeHeading bool) error {
	r, ok := d.body.(*richBody)
	if !ok {
		return ErrNotSupported
	}
	return r.ReplaceBlock(id, text, includeHeading)
}

func (d *IR) SetBlocks(blocks []Block) {
	if r, ok := d.body.(*richBody); ok {
		r.SetBlocks(blocks)
	}
}
